...
4. **llm-guard-proxy.service**
   A Rust-based shielding gateway proxy ([llm-guard-proxy](https://github.com/RyderFreeman4Logos/llm-guard-proxy)) sitting in front of the chat, embedding, and reranker endpoints. It routes requests by `model` to named upstream profiles, manages request queues, retries, stalls, and loop guards to protect backends from runaway generations. It owns the stable entrypoint `18009`, aggregate listener `18005`, and legacy restricted listeners `18002`/`18003`; raw vLLM backends stay on `18010`/`18012`/`18013`. It is also the runtime control plane for request concurrency and the sole automatic low-memory recovery actor. To tune limits or the `[guardian]` policy, edit the profile-owned source `profile/aeon-ultimate-uncensored-nvfp4/llm-guard-proxy/config.toml`, then atomically replace `/home/obj/.config/llm-guard-proxy/config.toml` (in-place `sed` is rejected). Generation limits hot-reload when topology is unchanged; restarting Guard is an optional fallback. Never restart vLLM for this knob.

   Queueing belongs primarily in Guard, not in an unbounded raw model adapter. The reference profile permits eight concurrent body-routing reads and queues 128 requests before model routing; after routing it allows 8 active + 56 queued AEON requests, 8 + 64 embedding requests, and 8 + 64 Querit requests. Queued requests may wait up to 30 minutes. Only the 128-slot body-routing wait is pre-body and cheap; profile queues retain request bodies, so Guard caps every request at 4 MiB. The worst-case 233 body residencies use a documented 384 MiB baseline plus 1.5× body-overhead budget (1,782 MiB), below `MemoryHigh=1792M` and `MemoryMax=2G`. Querit vLLM uses the live-proven AEON scheduler ceilings (`--max-num-batched-tokens 16384`, `--max-num-seqs 32`), while Guard's lower profile limit controls actual production concurrency and is hot-reloadable when topology is unchanged. The Ultimate `:18010` source unit keeps a 16-sequence scheduler ceiling (Dynamic DFlash2 K=10 lattice). The prior K=7/`max-num-seqs 32` contract is superseded.
...
ultimate override: sha256:0652d5b5641f673c43455523ceb981e8ddd4df04ad862ad86edb0d59a517672e
