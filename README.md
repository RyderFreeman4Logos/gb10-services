...
3. **vllm-querit-4b-reranker.service**
   The single canonical production owner for `Querit/Querit-4B` is a BF16 vLLM pooling reranker with the `qwen3-reranker-8b` and `Qwen/Qwen3-Reranker-8B` aliases, a 32,768-token context, 4,800 MiB KV cache, and equal 18 GiB Docker memory/swap caps. Its raw backend listens on `18013` with the live-proven AEON scheduler profile (`--max-num-batched-tokens 16384`, `--max-num-seqs 32`, `--max-num-partial-prefills 1`, and `--max-long-partial-prefills 1`). Every startup completes the bounded rerank-readiness probe before its unit-owned generation verifier runs; the verifier binds the exact Docker generation without querying the still-starting `Type=simple` service's active state. Clients should use Guard on `18009` or the restricted listener `18003`. `vllm-qwen3-reranker-8b.service` remains a disabled fallback only.
