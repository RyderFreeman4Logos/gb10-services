...
/** Do not present a last observed vLLM rate as current after this window. */
const VLLM_RATE_STALE_WINDOW_MS = 30_000;
...
export class LlmProbe {
  constructor(spark, port = 8888) {
    ...
    // Counter baselines stay at the last advance so a sparse flush gets one honest window.
    this._vllmRates = {
  ...
  /** Reset sparse-vLLM rate windows after a counter reset or target reset. */
  _resetVllmRates() {
    this._vllmRates = {
      generation: { counter: null, observedAt: null, updatedAt: null, value: 0 },
      prefill: { counter: null, observedAt: null, updatedAt: null, value: 0 },
  ...
  /**
   * Record a cumulative counter advance as one measured window. Zero deltas do
   * not refresh it: they are stale evidence, not evidence of current work.
   * @param {"generation" | "prefill"} kind
  ...
  _getSnapshot() {
    ...
    const generationRate =
      this.backendType === "vllm" ? this._vllmRateStatus("generation", Date.now()) : null;
    const prefillRate =
      this.backendType === "vllm" ? this._vllmRateStatus("prefill", Date.now()) : null;
    return {
      ...
      generationTps: generationRate?.value ?? this.generationTps,
      prefillTps: prefillRate?.value ?? this.prefillTps,
      generationTpsState: generationRate?.state ?? null,
      prefillTpsState: prefillRate?.state ?? null,
      generationTpsAgeSeconds: generationRate?.ageSeconds ?? null,
      prefillTpsAgeSeconds: prefillRate?.ageSeconds ?? null,
  ...
  _defaultLlm() {
    return {
      ...
      generationTps: 0,
      prefillTps: 0,
      generationTpsState: "unavailable",
      prefillTpsState: "unavailable",
      generationTpsAgeSeconds: null,
      prefillTpsAgeSeconds: null,
