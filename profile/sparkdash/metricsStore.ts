  ...
  for (const s of sparks) {
    ...
    if (Array.isArray(m.llm)) {
      ...
      for (let i = 0; i < m.llm.length; i++) {
        ...
        // Do not draw a held observation as live throughput; stale/unavailable
        // samples leave a visible time gap until the next fresh/idle reading.
        if (llm.generationTpsState !== "stale" && llm.generationTpsState !== "unavailable") {
          pushHistory(`${s.id}:llm${portKey}.tps`, llm.generationTps, at);
        }
        if (llm.prefillTpsState !== "stale" && llm.prefillTpsState !== "unavailable") {
