...
function rateLabel(value: number, state?: LlmMetrics["generationTpsState"], ageSeconds?: number | null): string {
  if (state === "unavailable") {
    return ageSeconds == null ? "unavailable" : `unavailable · ${ageSeconds.toFixed(1)}s`;
  }
  if (state === "stale") return `${value.toFixed(1)} · stale ${ageSeconds?.toFixed(1) ?? "?"}s`;
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="text-right">
                <div className="font-tabular text-sm font-semibold text-accent">
                  {rateLabel(generationTps, llm?.generationTpsState, llm?.generationTpsAgeSeconds)}
                </div>
                {genAvg != null && (
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="text-right">
                <div className="font-tabular text-sm font-semibold text-text">
                  {rateLabel(prefillTps, llm?.prefillTpsState, llm?.prefillTpsAgeSeconds)}
                </div>
                {prefillAvg != null && (
                  <div className="font-tabular text-[9px] text-muted">
