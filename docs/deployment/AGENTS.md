...
* `vllm-querit-4b-reranker.service`: single canonical BF16 pooling production owner on `18013`, with a 32,768-token context, 4,800 MiB KV cache, equal 18 GiB Docker memory/swap caps, and the live-proven AEON scheduler profile of 4,096 batched tokens / 16 sequences with both partial-prefill limits set to one. Its only post-start action is the bounded rerank-readiness probe; static no-swap validation runs before launch and the migration owner performs generation-bound verification after start.
