...
* Candidate `vllm-aeon-ultimate-uncensored-nvfp4.service` is the not deployed replacement for the :18010 text owner: `AEON-ULTIMATE-UNCENSORED-NVFP4-beta-version`, DFlash n=7, `max-model-len=262144`, and a 72G no-swap text cgroup.
...
* `llm-guard-proxy` routes by request `model` to AEON chat (`aeon`, `aeon-ultimate`, `abliterated-qwen-latest-27b-nvfp4`, `qwen3.6-27b-decensor-by-aeon`, `qwen3.6-27b-decensored`), embedding (`qwen3-embedding-8b`, `Qwen/Qwen3-Embedding-8B`), or reranker (`qwen3-reranker-8b`, `Qwen/Qwen3-Reranker-8B`).
